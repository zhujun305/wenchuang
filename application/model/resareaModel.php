...
class resareaModel extends commonModel
{
    static $table_name = 'res_area';
     ...
     * 根据where条件查询一条数据
     */
    static public function getOneByWhere($where=array(), $field=array(), $order="id asc")
    {
     ...
     * 查询分页列表
     */
    static public function getList($findObj=[], $field=[], $page="15", $order="id asc")
    {
    	$where = [];
		if(!empty($findObj)){
			if(!empty($findObj['name'])) $where['name'] = ['like', '%'.$findObj['name'].'%'];
			if($findObj['pid']>0) $where['pid'] = $findObj['pid'];
		}
     ...
     * 根据where条件查询列表
     */
    static public function getListByWhere($where=[], $field=[], $order="id asc", $limit='')
    {
