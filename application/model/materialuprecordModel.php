...
class materialuprecordModel extends commonModel
{
    static $table_name = 'material_uprecord';
...
//     	$return = Db::table(self::$table_name)->where($where)->field($field)->find();
    ...
    static public function getList($findObj = array(), $field = array(), $page='15', $order="id desc")
    {
    	$where = [];
		if(!empty($findObj)){
			if(!empty($findObj['filename'])) $where['filename'] = ['like', '%'.$findObj['filename'].'%'];
			if(isset($findObj['uid']) && $findObj['uid']>0) $where['uid'] = ($findObj['uid']);
			if(isset($findObj['is_daoru']) && $findObj['is_daoru']>0) $where['is_daoru'] = ($findObj['is_daoru']);
			if(isset($findObj['is_shangc']) && $findObj['is_shangc']>0) $where['is_shangc'] = ($findObj['is_shangc']);
		}
