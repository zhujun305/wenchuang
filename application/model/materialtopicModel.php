...
class materialtopicModel extends commonModel
{
    static $table_name = 'material_topic';
    ...
    static public function getList($findObj = array(), $field = array(), $page='15', $order="id desc")
    {
		...
		if(!empty($findObj)){
			...
			if(isset($findObj['is_recommend']) && $findObj['is_recommend']>0){
				$where['is_recommend'] = $findObj['is_recommend'];
			}
			if(isset($findObj['is_lock']) && $findObj['is_lock'] > 0){
				$where['is_lock'] = intval($findObj['is_lock']);
			}
		}
