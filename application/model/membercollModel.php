...
class membercollModel extends commonModel
{
    static $table_name = 'member_coll';
    ...
    static public function getList($findObj=array(), $field=array(), $page="15", $order="id desc")
    {
    	$where = [];
		if(!empty($findObj)){
			if(isset($findObj['cate']) && $findObj['cate']!=''){
				$where['cate'] = ($findObj['cate']);
			}
			if(isset($findObj['uid']) && $findObj['uid']>0){
				$where['uid'] = ($findObj['uid']);
			}
			if(isset($findObj['otherid']) && $findObj['otherid']>0){
				$where['otherid'] = ($findObj['otherid']);
			}
